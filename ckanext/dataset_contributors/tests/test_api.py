...
from ckanext.dataset_contributors.tests import helpers
...
class TestOrcidApi(TestBase):
    ...
    def _set_config_value(self, key, has_var, test_value=None):
        if has_var:
            self.config.update({
                u'ckanext.dataset_contributors.' + key: test_value
                })
        else:
            self.config.remove(u'ckanext.dataset_contributors.' + key)
    ...
    @mock.patch('ckanext.dataset_contributors.lib.orcid_api.requests.post', return_value=Response(ok=False))
