...
class OrcidApi(object):
    def __init__(self):
        self.key = toolkit.config.get(u'ckanext.orcid_datasets.key')
        self.secret = toolkit.config.get(u'ckanext.orcid_datasets.secret')
        self._debug = toolkit.config.get(u'ckanext.orcid_datasets.debug', True)
    ...
    def read_token(self):
        if self.key is None or self.secret is None:
            raise Exception(toolkit._(u'ORCID API credentials not supplied.'))
        r = requests.post('https://sandbox.orcid.org/oauth/token', data={
            'client_id': self.key,
    ...
    def search(self, surname_q=None, orcid_q=None):
        query = []
    ...
    def as_contributor_record(self, orcid_record):
        ...
        return {
            ...
            u'affiliations': [e.get(u'organization', {}).get(u'name', None) for e in employments if
